...
{
    std::unordered_map<Node_t*, Node_t*> src_to_dest;            // really wanted to include unordered_map for constant time lookups, helping performance 

    Node_t* new_list = list;
    while (new_list) //first pass allocates memory for a new node for each node in original list
    {
        src_to_dest[new_list] = (Node_t*)malloc(sizeof(Node_t)); // would use new Node_t() when in C++ but the src list is allocated with malloc
                                                                 // so i wanted to keep cohesion.
        assert(src_to_dest[new_list] != NULL);              
        new_list = new_list->next;
    }

    new_list = list;
    while (new_list) //second pass assigns next and reference pointers to corresponding new node 
    {
        src_to_dest[new_list]->next = src_to_dest[new_list->next];
        src_to_dest[new_list]->reference = src_to_dest[new_list->reference];
        new_list = new_list->next;
    }

    return src_to_dest[list];
}


//XX
static void FreeList(Node_t* head) { //iterate thru linked list and free each node
    Node_t* cursor = head;
    while (cursor != NULL) {
